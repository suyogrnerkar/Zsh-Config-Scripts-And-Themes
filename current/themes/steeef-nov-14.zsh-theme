...
ZSH_THEME_GIT_TIME_SINCE_COMMIT_LONG="%{$fg[white]%}"
...
function git_time_since_commit() {
    if git rev-parse --git-dir > /dev/null 2>&1; then
        ...
        if [[ $(git log 2>&1 > /dev/null | grep -c "^fatal: bad default revision") == 0 ]]; then
            ...
            if [ "$HOURS" -gt 24 ]; then
                echo "\nᗒ %{$white%}Git status :($(rvm_gemset)$COLOR${DAYS}d;${SUB_HOURS}h;${SUB_MINUTES}m%{$reset_color%}|"
            elif [ "$MINUTES" -gt 60 ]; then
                echo "\nᗒ %{$white%}Git status :($(rvm_gemset)$COLOR${HOURS}h${SUB_MINUTES}m%{$reset_color%}|"
            else
                echo "\nᗒ %{$white%}Git status :($(rvm_gemset)$COLOR${MINUTES}m%{$reset_color%}|"
            fi
        else
            COLOR="$ZSH_THEME_GIT_TIME_SINCE_COMMIT_NEUTRAL"
            echo "\nᗒ %{$white%}Git status :($(rvm_gemset)$COLOR~|"
        fi
    fi
}
